...
exports.recommendedUnsplash = exports.searchUnsplash = exports.movieSearch = exports.songSearch = exports.gameSearch = exports.searchBook = void 0;
const zod_1 = __importDefault(require("zod"));
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
const bookschema = zod_1.default.object({
    name: zod_1.default.string().min(1, "Title is required"),
});
const searchBook = async (req, res) => {
    ...
    try {
        ...
        const cleaned = data.items.map((book) => ({
            id: book.id,
...
const songSearch = async (req, res) => {
    try {
        ...
        const url = `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=track,album,artist&limit=10`;
        ...
        const cleaned = songs.tracks.items.map((track) => ({
            id: track.id,
            name: track.name,
...
const movieSearch = async (req, res) => {
    ...
    try {
        ...
        const data = await fetch(`https://imdb.iamidiotareyoutoo.com/search?q=${movie}`, {
            method: "GET",
        });
        if (!data.ok) {
        ...
        const cleaned = description.map((movie) => ({
            id: movie["#IMDB_ID"],
...
const searchUnsplash = async (req, res) => {
    try {
        ...
        const data = await response.json();
        console.log(data);
        const cleaned = data.results.map((img) => ({
            id: img.id,
...
exports.searchUnsplash = searchUnsplash;
const recommendedUnsplash = async (req, res) => {
    try {
        const response = await fetch(`https://api.unsplash.com/photos?per_page=10&order_by=popular`, {
            headers: {
                Authorization: `Client-ID ${process.env.UNSPLASH_ACCESS_KEY}`,
            },
        });
        const data = await response.json();
        const cleaned = data.map((img) => ({
            id: img.id,
            title: img.description,
            description: img.description || img.alt_description,
            url: img.urls?.regular,
...
exports.recommendedUnsplash = recommendedUnsplash;
